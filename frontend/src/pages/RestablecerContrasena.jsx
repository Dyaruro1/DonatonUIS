...
function RestablecerContrasena() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (resp && resp.nuevaContrasena) {
        setNuevaContrasena(resp.nuevaContrasena);
      }
      setEnviado(true);
    } catch (err) {
      ...
      >
        ...
        {enviado ? (
          <div style={{textAlign: 'center', color: '#21E058', fontWeight: 500, margin: '1.5rem 0'}}>
            ¡Revisa tu correo para continuar con el restablecimiento!<br />
            {nuevaContrasena && (
              <>
                <div style={{color: '#222', marginTop: '1.2rem', fontWeight: 400}}>
                  <b>Tu nueva contraseña temporal es:</b><br />
                  <span style={{fontSize: '1.15rem', color: '#21E058'}}>{nuevaContrasena}</span><br />
                  <span style={{fontSize: '0.98rem', color: '#444'}}>Cámbiala después de iniciar sesión.</span>
                </div>
              </>
            )}
          </div>
